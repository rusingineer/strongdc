...
private:
	HANDLE  hMapFile;   					// mapping file object
	LPVOID  lpMapAddr;  					// current mapping start address

	int64_t iMapPos;						// iPos align value
	int64_t iPos;							// current write position
	int64_t iFileSize;  					// file total size
	DWORD   dwMapSize;  					// current mapping size
	DWORD   dwBuffSize; 					// use expected buffer size
	DWORD   dwSysGran;						// system allocation granularity

public:
	MappedFile(const string& aFileName, int64_t filesize, int access, int mode, bool /*aCrc32*/ = false, 
	           int bufSize = SETTING(BUFFER_SIZE)) throw(FileException) : 
		File(aFileName, access, mode), iPos(0), lpMapAddr(NULL), iFileSize(filesize){
		...
		dwBuffSize = (bufSize * 1024 / dwSysGran + 1) * dwSysGran;
        dwMapSize  = 0;
        iMapPos    = 0;
	...
	virtual ~MappedFile() throw(FileException) {
		close();
	}
	...
	virtual int64_t getSize() throw(FileException) { return iFileSize; }
	virtual int64_t getPos() throw(FileException) { return iPos; }
	virtual void setPos(int64_t aPos) throw(FileException) { iPos = aPos; }
	virtual void setEndPos(int64_t) throw(FileException) {}
	virtual void movePos(int64_t) throw(FileException) {}
	virtual u_int32_t read(void* , u_int32_t) throw(FileException) {return 0;}
	virtual void setEOF() throw(FileException) {}

	virtual size_t write(const void* buf, size_t len) throw(Exception);
	//virtual void write(const void* aBuf, u_int32_t len) throw(FileException) ;

	unsigned int SafeCopyMemory(LPBYTE lpDst, LPCVOID lpSrc, DWORD dwLen)
	{
		__try
		{
			::CopyMemory(lpDst, lpSrc, dwLen);
		}
