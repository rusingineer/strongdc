...
void DownloadManager::on(TimerManagerListener::Second, u_int32_t /*aTick*/) throw() {
	...
	for(Download::Iter i = downloads.begin(); i != downloads.end(); ++i) {
		...
		if(q && SETTING(SPEED_USERS) && !q->getFastUser() && q->getNoFreeBlocks() && (q->speedUsers.size() > 0) && (q->getActiveSegments().size() == 1)) {
			...
			if((d->getRunningAverage() < (q->speedUsers[0]->getUser()->getDownloadSpeed() / 2)) && (QueueManager::getInstance()->getRunning(q->speedUsers[0]->getUser()) == NULL)) {
				TryToSwitchToUser = 0;
			} else if((q->speedUsers.size() > 1) && (d->getRunningAverage() < (q->speedUsers[1]->getUser()->getDownloadSpeed() / 2)) && (QueueManager::getInstance()->getRunning(q->speedUsers[1]->getUser()) == NULL)) {
				TryToSwitchToUser = 1;
			} else if((q->speedUsers.size() > 2) && (d->getRunningAverage() < (q->speedUsers[2]->getUser()->getDownloadSpeed() / 2)) && (QueueManager::getInstance()->getRunning(q->speedUsers[2]->getUser()) == NULL)) {
				TryToSwitchToUser = 2;
			}
			if((TryToSwitchToUser > -1) && q->speedUsers[TryToSwitchToUser]->getUser()->isOnline() && (d->getUserConnection()->getUser() != q->speedUsers[TryToSwitchToUser]->getUser())) {
			...
			for(Download::Iter i = downloads.begin(); i != downloads.end(); ++i) {
				...
				if(d != old && d->getFile() != NULL && false == d->isSet(Download::FLAG_USER_LIST) && d->isSet(Download::FLAG_TREE_DOWNLOAD) == false 
					...
					if(d->isSet(Download::FLAG_ZDOWNLOAD)) {
						...
						FilteredOutputStream<UnZFilter, true>* unZFilter = (FilteredOutputStream<UnZFilter, true>*)d->getFile();
						ChunkOutputStream<true>* f = (ChunkOutputStream<true>*)unZFilter->getOutputStream();
						unZFilter->setOutputStream(new TigerCheckOutputStream<true>(d->getTigerTree(), f, f->getPos(), d->getTempTarget()));
					}else{
						dcdebug("  Set tree to %s \n", d->getSource().c_str());
						ChunkOutputStream<true>* f = (ChunkOutputStream<true>*)d->getFile();
						d->setFile(new TigerCheckOutputStream<true>(d->getTigerTree(), f, f->getPos(), d->getTempTarget()));
					}
				}
			}
		}
