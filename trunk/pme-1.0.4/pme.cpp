...
//
// Distribution under the GPL or LGPL overrides any other restrictions, as with the PCRE license
//
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    )
{
    ...
    // store off the string we compiled for future reference
    m_sRegexString = s;
    ...
    )
{
    ...
    // fprintf ( stderr, "Matching against regex: %s\n", GetRegexString ( ).c_str ( ) );

    // determine the number of backrefs possilbe and allocate an array for holding them (??)
    int msize;
    pcre_fullinfo(re, 0, PCRE_INFO_CAPTURECOUNT, &msize);
    // fprintf ( stderr, "Max back references: %d\n", msize );

    // allocate room for all the backrefs
    msize = 3*(msize+1);


    // m holds all the matches
    // fprintf ( stderr, "Allocating an array of integers %d elements long\n", msize );
    int * m = new int [ msize ];

    // fprintf ( stderr, "Done allocating the array" );
    ...
    // fprintf ( stderr, "pcre_exec result (m_nMatches) = %d\n", m_nMatches );
	...
	// fprintf ( stderr, "%p i*2=%d\n", p, i*2 );
	m_marks.push_back(markers(p[0], p[1]));
    }

    // get rid of the pcre-style matches now that we have them for ourselves
    delete[] m;
...
PME::split(const std::string & s, unsigned maxfields)
{
    ...
    if ( nMatchesFound == 0 ) {
	// fprintf ( stderr, "Putting the whole thing in..\n" );
    ...
    //   onto the end of the last split field
	      ...
	      nMatchesFound >= maxfields ) {
	// fprintf ( stderr, "Pushing the rest of the string into the last split field" );
	oMarks [ oMarks.size ( ) - 1 ].second = s.length ( );
    } 
    // else we have to add another entry for the end of the string
    else {
	// fprintf ( stderr, "Not sure what this is..\n" );
